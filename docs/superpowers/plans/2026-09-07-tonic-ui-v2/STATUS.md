...
| BR0 | [可重跑 browser baseline](details/09a-browser-procedure.md) | H3 | in_progress | root session / 2026-09-07T22:43:05+08:00 | `183b42b6` plus fresh `port-selection/` evidence verify simulator startup, selector, `/tmp/ttyGRBL`, and initial Grbl connection. Browser upload exposed and now fixes `Visualizer.load()` argument ordering; focused and frontend Jest pass. `artifacts/browser/br0-playwright-cli/` confirms the small fixture no longer causes `str.split`/React overlay, but its WebGL modal dismissal chose the wrong disabled Close button, so it did not re-establish connection or verify controls. Run/Pause/Resume/Stop/jog/disconnect and fixed large fixtures remain pending. Do not use system Chrome channel for screenshots. |
