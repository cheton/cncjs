...
- [STATUS](STATUS.md)：BR0 為 waived；R0 non-browser baseline、D1、D2、D3、D4、R1、R2、R3 已完成；browser gaps 依 waiver 延後至 R6。R1 新增 16 個真 frame shell 的 contract tests；R2 新增 Workspace group/lifecycle regression coverage；R3 新增 real Three.js/GCodeVisualizer geometry and pivot baselines。工作樹目前保留 R3 tests 與 ledger updates，準備 phase commit；runtime-generated large G-code/watch-tree payload 不追蹤。BR0-B06 已解決，BR0-B07/B08 與 `br0-20260913-200600` evidence 已保存。
...
目前 **沒有** `in_progress`、也沒有未解 blocker。R1、R2、R3 regression gates 已完成；**U2** 是唯一可立即開跑的 implementation task（依賴 R1 + R2），其餘 todo 都還被 U2/U3 或後續未完成依賴擋住。

| 可執行 task | Depends on | 性質 | 需要 browser？ |
| --- | --- | --- | --- |
| **U2** [primitives pilot](02-shared-ui.md) | R1 ✅, R2 ✅ | 第一個可執行 implementation task；解鎖 U3 與後續 widget work | 否（unit） |

R1、R2、R3 已完成。下一步是 U2（primitives pilot）；U2 依賴 R1 + R2，會解鎖 U3 與後續 widget implementation。

**要從哪裡開始？** R1、R2、R3 已完成。下一步是 U2；它是目前唯一可立即開跑的 implementation task。
...
優先恢復 in_progress；目前沒有 in_progress。依 STATUS 依賴計算，R1/R2/R3 已完成；目前第一個可開跑的 implementation task 是 U2，不能領取 U3/T1 或其他仍被依賴擋住的 task。若要 waived dependency 的下游，依 STATUS 的 waiver scope 繼續。
