...
- Active task: **A1b Axes input** — report normalization and listener cleanup are unit-tested; Axes presentation components consume a provider instead of an `actions` bag. The remaining work is a real function/reducer/effect owner and full command/cleanup coverage.
...
| A1b | [Axes input](06-motion-widgets.md) | S4, G4 | in_progress | current root session / 2026-09-20 | Provider supplies state and named commands to Axes/DisplayPanel/Keypad/MDI consumers, replacing their `actions` bag; shared MDI query, direct Tonic editable position primitives, hotkey guard, pure report normalization, metric/imperial distance helper, function Keypad, and paired listener cleanup are implemented. Broader focused Axes/Settings/Custom/Probe run: 8 suites / 39 tests; current Axes slice: 11 tests; targeted ESLint and diff check pass. Remaining: convert the class owner to a real function/reducer/effect lifecycle and add required command, disconnect, and unmount cleanup coverage. Browser evidence remains deferred to R6. |
