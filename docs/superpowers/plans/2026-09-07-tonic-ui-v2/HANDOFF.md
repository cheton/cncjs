...
- Mode: **layout naming/API cleanup, B1 session boundary, and M1 Macro query contract complete; implementation ready for the next task with BR0 waived**。使用者明確允許不要卡在 BR0；BR0 保留為未完成 browser evidence 的 accepted risk，不標示 completed。R0 的非 browser baseline、D1 pure widget layout state、D2 `WorkspaceLayoutProvider`/hydration、D3 `WidgetHost`/16 個 layout-aware consumers、D4 Workspace/group wiring、B1 session/query boundary，以及 M1 shared Macro query contract 已完成；B1 and M1 phase deliveries are committed on the current feature branch。現行 API 是 `view`（`normal`／`collapsed`／`fullscreen`）與 `onViewChange(view)`；`minimized` 僅保留為既有 config persistence key，fullscreen 不寫入 config。現有 BR0 evidence 證明 connection、small upload、Run/Pause/Resume，Stop、jog、disconnect、large fixture、watch-tree、viewport 與新 selector browser evidence 延後至 R6。
...
- [STATUS](STATUS.md)：BR0 為 waived；R0 non-browser baseline、D1、D2、D3、D4、R1、R2、R3、U2、U3、B1、M1 已完成，browser gaps 依 waiver 延後至 R6。U3 完成 Custom SettingsModal 的 direct Tonic overlay/form contract 與六個 interaction tests；`useToast` 已是 Tonic Toast 並保持既有 persistence semantics。B1 完成 session mutation、pending/error login handling、logout cache ordering、以及 bootstrap pure transport reuse。M1 完成 shared Macro list/detail/CRUD query hooks, Axios signal propagation, stable key/options contracts, and invalidation ordering。R1 新增 16 個真 frame shell 的 contract tests；R2 新增 Workspace group/lifecycle regression coverage；R3 新增 real Three.js/GCodeVisualizer geometry and pivot baselines；U3、B1、M1 phase deliveries 已 committed；runtime-generated large G-code/watch-tree payload 不追蹤。BR0-B06 已解決，BR0-B07/B08 與 `br0-20260913-200600` evidence 已保存。
...
目前 **沒有** `in_progress`、也沒有未解 blocker。R1、R2、R3、U2、U3、B1、M1 已完成；M1 已解除後續 Macro mutation work，其餘 todo 仍被後續依賴擋住。
...
R1、R2、R3、U2、U3、B1、M1 已完成。下一個推薦 task 是 **T1**，因為它建立 Terminal owner baseline；P0 也已符合依賴，可在不共享修改時平行評估。

**要從哪裡開始？** 先領取 **T1**（推薦），或選擇已解鎖的 P0。
...
優先恢復 in_progress；目前沒有 in_progress。依 STATUS 依賴計算，R1/R2/R3/U2/U3/B1/M1 已完成；目前推薦先領取 T1，也可領取 P0，不能跳過各自仍未完成的前置 task。若要 waived dependency 的下游，依 STATUS 的 waiver scope 繼續。
