...
/**
 * @param {{
 *   initialValues: { name: string, command: string, grid?: { xs?: number } },
 *   onSave: (record: object) => void,
 *   onCancel: () => void
 * }} props
 * @returns {JSX.Element}
 */
function UpdateRecord({ initialValues, onSave, onCancel }) {
  const [name, setName] = useState(initialValues.name || '');
  const [command, setCommand] = useState(initialValues.command || '');
  const [grid, setGrid] = useState(initialValues.grid?.xs || 6);
