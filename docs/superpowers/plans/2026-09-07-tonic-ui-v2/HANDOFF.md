...
- Mode: **layout naming/API cleanup, B1 session boundary, M1 Macro query contract, and T1–T3 Terminal work complete; implementation ready for the next task with BR0 waived**。使用者明確允許不要卡在 BR0；BR0 保留為未完成 browser evidence 的 accepted risk，不標示 completed。R0 的非 browser baseline、D1 pure widget layout state、D2 `WorkspaceLayoutProvider`/hydration、D3 `WidgetHost`/16 個 layout-aware consumers、D4 Workspace/group wiring、B1 session/query boundary、M1 shared Macro query contract，以及 T1–T3 Terminal work 已完成；B1, M1, T1, T2, and T3 phase deliveries are committed on the current feature branch。現行 API 是 `view`（`normal`／`collapsed`／`fullscreen`）與 `onViewChange(view)`；`minimized` 僅保留為既有 config persistence key，fullscreen 不寫入 config。現有 BR0 evidence 證明 connection、small upload、Run/Pause/Resume，Stop、jog、disconnect、large fixture、watch-tree、viewport 與新 selector browser evidence 延後至 R6。
...
目前 **沒有** `in_progress`、也沒有未解 blocker。R1、R2、R3、U2、U3、B1、M1、T1、T2、T3 已完成；G8 Terminal work is complete, and P0 is the remaining dependency-eligible task in this lane.
...
R1、R2、R3、U2、U3、B1、M1、T1、T2、T3 已完成。G8 Terminal work is complete. 下一個推薦 task 是 **P0**；它可在不共享 Terminal 修改時直接進行。
...
優先恢復 in_progress；目前沒有 in_progress。依 STATUS 依賴計算，R1/R2/R3/U2/U3/B1/M1/T1/T2/T3 已完成；目前推薦先領取 P0，不能跳過各自仍未完成的前置 task。若要 waived dependency 的下游，依 STATUS 的 waiver scope 繼續。
